...
function formatDecls (rule, indent, indentWidth, stylelint) {
  var propNoVendorPrefix
  var valueNoVendorPrefix
  function clearPrefixedDecl (decl) {
    ...
    prefixedDecls.forEach(function (decl) {
      if((propNoVendorPrefix && valueNoVendorPrefix) || (propNoVendorPrefix && decl.prop[0] === '-') || (valueNoVendorPrefix && /(^|,|\s)-\w+-.+/.test(decl.value))){
        decl.remove()
      }
    })
  }
  ...
  if (hasDecls(rule)) {
    propNoVendorPrefix = getProperty(stylelint, 'property-no-vendor-prefix')
    valueNoVendorPrefix = getProperty(stylelint, 'value-no-vendor-prefix')
