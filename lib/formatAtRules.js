...
function formatAtRules (root, params) {
  ...
  root.walkAtRules(function (atrule, index) {
    ...
    if (atRuleEmptyLineBefore) {
      ...
      if(options.indexOf('first-nested') > -1) {

        if (isFirstNested(prev, atrule, isNested)) {
      ...
      if(options.indexOf('blockless-after-same-name-blockless') > -1) {

        if (isBlocklessAfterSameNameBlockless(prev, atrule)) {
          atRuleEmptyLineBefore = !atRuleEmptyLineBefore
        }
      }
