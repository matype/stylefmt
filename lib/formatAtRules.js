...
function formatAtRules (root, params) {
  ...
  root.walkAtRules(function (atrule, index) {
    ...
    if (atRuleEmptyLineBefore) {
      ...
      if ( (options.indexOf('all-nested') > -1 && isNested)
        || (options.indexOf('first-nested') > -1 && isFirstNested(prev, atrule, isNested))
        || (options.indexOf('blockless-group') > -1 && isBlocklessGroup(prev, atrule))
        || (options.indexOf('blockless-after-same-name-blockless') > -1 && isBlocklessAfterSameNameBlockless(prev, atrule)))
      {
