...
function loadConfig (stylelint, file, configFile) {
  return Promise.resolve().then(function (){
    if (file && !stylelint._options.ignoreDisables) {
      return stylelint.isPathIgnored(file)
    }
  }).then(function (isIgnored) {
    if (isIgnored) {
      return null
    }
    ...
    var fileNotFound = err.code === 'ENOENT' && path.resolve(process.cwd(), stylelint._options.codeFilename) === err.path
...
function params (options) {
  ...
  return function (root, result) {
    ...
    return loadConfig(stylelint, file, options.configFile)
  }
}
