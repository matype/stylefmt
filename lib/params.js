...
function loadStylelint(from) {
  var ret = {}
  var configNames = ['stylelint', 'stylelint.config']
  configNames.forEach(function (configName) {
    assign(ret, rc(configName, {}, { config: path.join(from, configName) }).rules)
  })
  return ret
}
...
function newParmas(from) {
  ...
  var indentation = stylelintConfig['indentation']
  switch (typeof indentation) {
    case 'string':
      params.indentWidth = '\t'
      break
    case 'number':
      params.indentWidth = repeatString(' ', indentation)
      break
    default:
