...
function formatRules (root, params) {
  ...
  root.walkRules(function (rule, index) {
    ...
    var indent = getIndent(rule, indentWidth)
    ...
    rule = formatSelectors(rule, indent, stylelint)
    rule = formatDecls(rule, indent, indentWidth)

    rule.walkAtRules(function (rule) {
      var indent = getIndent(rule, indentWidth)
      formatDecls(rule, indent, indentWidth)
    })
  })
